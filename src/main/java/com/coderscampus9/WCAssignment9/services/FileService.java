package com.coderscampus9.WCAssignment9.services;
...
////+ "leaving skin on the baking sheet.", 5.0, 427.92, 15, 4, 99.0, "Roasted Salmon With Lime And Cilantro", false, false
	...
	private List<Recipe> readRecipes (String fileSource) {
	...
	public List<Recipe> getGlutenFree (String sourceFile) {
		return readRecipes(sourceFile).stream()
									  .filter(recipe -> recipe.getGlutenFree() == true)
									  .toList();
	}
	
	public List<Recipe> getVegan (String sourceFile) {
		return readRecipes(sourceFile).stream()
									  .filter(recipe -> recipe.getVegan() == true)
									  .toList();
	}
	
	public List<Recipe> getVeganAndGlutenFree (String sourceFile) {
		return readRecipes(sourceFile).stream()
									  .filter(recipe -> recipe.getVegan() == true && recipe.getGlutenFree() == true)
									  .toList();
	}
	
	public List<Recipe> getVegetarian (String sourceFile) {
		return readRecipes(sourceFile).stream()
									  .filter(recipe -> recipe.getVegetarian() == true)
									  .toList();
	}
	
	public List<Recipe> getAllRecipes (String sourceFile) {
